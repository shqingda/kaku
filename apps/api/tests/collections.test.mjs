...
test('entity collection reads v0 state and writes through P1 collection routes', async () => {
  const requests = [];
  let readCount = 0;
  const fetcher = async (input, init = {}) => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      {
        method: 'PUT',
        path: '/p1/collections/characters/1',
      },
      { method: 'DELETE', path: '/p1/collections/persons/2' },
    ],
  );
});
