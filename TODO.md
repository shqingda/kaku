...
- [ ] 本地/云端统计与年度报告：已将收藏与最近浏览合并为“我的数据”；用紧凑比例图展示媒体类型与按类型切换的收藏状态，并给出总数、进行中、待开始、已完成、搁置和放弃；只读取每类第一页的公开 `total`，年度趋势仍需完整历史数据后再做。
- [x] 本地浏览摘要：已并入“我的数据”，基于同步后的最近 10 条浏览记录展示近期条目数、涉及类型、主要类型、最新时间和媒体类型比例图；不再展示样本过小、意义有限的活跃日期，也不增加云端数据采集。
- [x] 导入/导出收藏与笔记：除汇总分享外，现可分页导出公开收藏条目为 JSON/CSV；导入为粘贴校验（明确不写回 Bangumi）。不含私密笔记（公开收藏接口没有评论字段）。登录且打开云同步后可把 JSON 存到 R2，保留 7 天、最多 5 份，失败显式重试。
...
- [x] R2：`kaku-exports` 仅按需保存用户主动上传的收藏备份（7 天 TTL、每用户最多 5 份、单份 ≤800KB）；对象不公开，取回需登录。抓取快照仍不做，以免把 HTML 落到存储里。
- [x] Queues：评估后暂缓。导出在客户端生成后按需上传，不走队列，也无需每日百万级操作；推送通知仍需要 Queue，等 APNs/FCM 一并做。
