...
- [x] 讨论页滚动导航（第三轮反馈后回滚定型）：小组话题、条目讨论版话题、吐槽箱、日志/长评回复与人物/角色评论弹层保留「跳到底部」单按钮 + 点击导航栏标题回顶，滚动用 `scrollToEnd`；吐槽箱点按钮触发下一页加载（页脚显示「正在加载更多结果」，加载后不自动滚动）。单集评论页回滚为「回到顶部」+「跳到底部」双按钮堆叠 + 原生标题（无标题点击回顶）。条目讨论版单个回复的举报按钮保持移除（话题级举报保留）。iOS 标题不对齐系撑满容器的自定义标题所致，回滚后由原生居中。
