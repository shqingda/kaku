...
- [x] EAS 项目初始化与账号绑定：已链接 `@shqingda/kaku`（projectId `65b88c02-a4de-4cfa-abbb-b4a2cbbd0861`），`eas.json` 的 `appVersionSource` 已切到 `remote`，本地 `versionCode`/`buildNumber` 已移除；Android 发布签名 keystore 由 EAS 远端管理。
