...
- [x] 讨论页滚动导航重构（第二轮反馈）：条目讨论版单个回复的举报按钮暂时移除（话题级举报保留，其他页面回复举报不变）；「回到顶部」扩大为点击整个导航栏（左右按钮除外）；「跳到底部」修复为一次直达真底部——此前 `scrollToEnd` 只能到虚拟渲染窗口末尾，全量加载的单集评论每次点按仅前进约一屏（即反馈中的 12→23→34 现象），现改用 `scrollToIndex(最后一条)` + 失败重试；吐槽箱点「跳到底部」触发下一页加载时页脚「正在加载更多结果」随请求出现（此前先滚动后请求、动画落点晚于请求完成所以几乎看不到）。
- [x] 讨论页/评论回复页「跳到底部」按钮：小组话题、条目讨论版话题、吐槽箱、日志/长评回复、单集评论与人物/角色评论弹层均可一键跳到最新回复；按钮除滚动外还会在内容尺寸变化时重算可见性，长话题一打开即可见；与原「回到顶部」按钮堆叠摆放。
- [x] 评论分页加载数量：吐槽箱每页 30 → 100（p1 上游上限，实测 limit>100 返回 400）；单集评论 p1 端点无分页参数、一次返回全部（官方 spec 无查询参数，实测 limit 被忽略），条目长评列表上游上限即 20。
