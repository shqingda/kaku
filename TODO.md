...
- [x] 讨论页滚动导航重构：小组话题、条目讨论版话题、吐槽箱、日志/长评回复、单集评论与人物/角色评论弹层删除「回到顶部」按钮，「跳到底部」按钮移到其原位；回到顶部改为点击顶部导航栏标题（人物/角色弹层保留点击抽屉标题回顶）。吐槽箱点「跳到底部」会顺势触发下一页加载（页脚显示加载状态，同吐槽箱既有机制），加载后不自动滚动，由用户继续滑动或再次点按。
